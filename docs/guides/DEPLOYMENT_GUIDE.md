...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`✅ WebSocket server ready at wss://api.getcourses.net`);
});
```
...
**File: `/etc/nginx/sites-available/api.getcourses.net`**
...
server {
    ...
    # SSL Configuration
    ssl_certificate /etc/letsencrypt/live/api.getcourses.net/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.getcourses.net/privkey.pem;
...
```bash
sudo ln -s /etc/nginx/sites-available/api.getcourses.net /etc/nginx/sites-enabled/
sudo nginx -t
...
server {
    ...
    # Redirect HTTP to HTTPS
    return 301 https://getcourses.net$request_uri;
}

server {
    listen 443 ssl http2;
    server_name www.getcourses.net;
...
# Test stats endpoint
curl https://api.getcourses.net/api/admin/stats
```
...
1. Open admin dashboard: `https://getcourses.net/admin`
...
5. Test WebSocket endpoint: `curl -i https://api.getcourses.net/socket.io/`
...
**Solution:**
1. Add `getcourses.net` to CORS origins in backend
...
4. Test API directly: `curl https://api.getcourses.net/api/admin/orders`

---

## ✅ Post-Deployment Checklist

- [ ] Frontend accessible at `https://getcourses.net`
- [ ] Admin dashboard accessible at `https://getcourses.net/admin`
- [ ] Backend API responding at `https://api.getcourses.net`
